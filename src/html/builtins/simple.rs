    ...
    CustomTy: Clone + 'static,
{
    ...
    fn open_tag(
        &self,
        tk_writer: &dyn crate::html::HtmlTokenWriter<CustomTy>,
...
macro_rules! simple_tag {
    ($doc:expr, $name:ident, $tags:expr, $open:expr, $close:expr) => {
        ...
        pub struct $name<CustomTy = ()> {
            _custom_ty: core::marker::PhantomData<CustomTy>,
        }
...
simple_tag! {
"An underline tag with no arguments, which converts directly to HTML5 `<u>`.",
UnderlineTag, ["u", "underline", "under"], "<u>", "</u>"
}
simple_tag! {
"A strikethrough tag with no arguments, which converts directly to HTML5 `<s>`.",
StrikethroughTag, ["s", "del", "strike"], "<s>", "</s>"
}
